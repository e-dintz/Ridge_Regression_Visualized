...
Next, you can compare the least squares fit of a high degree polynomial (typically overfit) with one ridge regression fit, for instance $\alpha=10$. The \texttt{sklearn} ridge regression package solves the optimization problem $\left\{\|\vec{y}-\vec{X}\vec{\beta}\|_{\ell^2}+\alpha\|\vec{\beta}\|_{\ell^2}\right\}$, in other it solves for a parameter vector that minimizes the residuals and the $\ell^2$ norm of the parameter vector $\vec{\beta}$.
<div align="center">
